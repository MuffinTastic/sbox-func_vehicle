...
public partial class PhysGun : Carriable, IUse
{
	...
	{
		if ( Owner is not FuncVehiclePlayer owner ) return;
	...
	private void TryUnfreezeAll( FuncVehiclePlayer owner, Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
	...
	private void TryStartGrab( FuncVehiclePlayer owner, Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		var tr = Trace.Ray( eyePos, eyePos + eyeDir * MaxTargetDistance )
			.UseHitboxes()
			.Ignore( owner, false )
		...
		//
		if ( body.BodyType == PhysicsBodyType.Keyframed && rootEnt is not FuncVehiclePlayer )
			return;
	...
	private void GrabMove( Vector3 startPos, Vector3 dir, Rotation rot, bool snapAngles )
	{
		...
		if ( GrabbedEntity is FuncVehiclePlayer player )
		{
